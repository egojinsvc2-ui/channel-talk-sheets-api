...
// 메모리에서 action_date, change_date, request 가져오기
const actionDate = memory.get('action_date') || '';
const changeDate = memory.get('change_date') || '';
const request = memory.get('request') || '';

console.log('[정보수집] 문의인입 기록 준비');
console.log(`   이름: ${name}`);
console.log(`   전화번호: ${mobileNumber}`);
console.log(`   기존일정: ${actionDate}`);
console.log(`   변경원하는일정: ${changeDate}`);
